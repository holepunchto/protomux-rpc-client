...
test('capability - valid capability', async (t) => {
  ...
  const res = await client.makeRequest(server.publicKey, 'echo', b4a.from('hello'), {
...
test('capability - invalid capability', async (t) => {
  ...
  await t.exception(async () => {
    await client.makeRequest(server.publicKey, 'echo', b4a.from('hello'), {
      requestEncoding: cenc.buffer,
      responseEncoding: cenc.buffer
    })
...
async function setupCapabilityServer(
  t,
  bootstrap,
  { namespace = undefined, capability = null } = {}
) {
