...
class ProtomuxRpcConnection extends SuspendResource {
  ...
  async _close() {
    ...
    this._requestConcurrentLimiter.destroy()
  }
  ...
  async _connect() {
    ...
    while (!this.closing && !this.suspended && !this.shouldBeSuspended) {
      ...
      // if capability is enabled, wait for the following handshake to complete
      if (this._capability) {
        await socket.opened // never throws
        if (socket.destroying || this.closing) continue
      }
      ...
      const rpc = new ProtomuxRPC(socket, {
        ...
        handshake: this._capability
          ? { capability: this._cap.generate(socket, this._capability) }
          : null
      })
      ...
      try {
        const handshake = await waitForRPC(rpc)
        if (this._capability) {
          if (
            !handshake?.capability ||
            !this._cap.verify(socket, this._capability, handshake.capability)
          ) {
            const err = Errors.INVALID_REMOTE_CAPABILITY()
            rpc.destroy(err)
            throw err
          }
        }
        ...
        if (err.code === 'INVALID_REMOTE_CAPABILITY') {
          // do not retry
          throw err
        }
  ...
  async makeRequest(methodName, args, { requestEncoding, responseEncoding, timeout = 10000 } = {}) {
    ...
    try {
      return await this._requestConcurrentLimiter.execute(
        () => {
          return this._connectAndSendRequest(methodName, args, {
            requestEncoding,
            responseEncoding,
            rpcTimeout: timeout,
            abortSignalPromise
  ...
  async event(methodName, args, { requestEncoding } = {}) {
    const timeout = 10000
    if (!this.opened) await this.ready()
  ...
  async _connectAndSendRequest(
    ...
    const res = await this.rpc.request(methodName, args, {
      requestEncoding,
      responseEncoding,
      timeout: expectedRpcTimeout
    })
