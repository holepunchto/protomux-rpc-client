  ...
  async _connect () {
    ...
    while (!this.closing && !this.suspended && !this.shouldBeSuspended) {
      ...
      if (this._capability) {
        rpc.on('open', (handshake) => {
          if (
            ...
            rpc.destroy(Errors.INVALID_REMOTE_CAPABILITY())
          }
        })
      }
      ...
      try {
        ...
        if (invalidCapability) { // no point in retrying
          // DEVNOTE: would be cleaner if we can re-use the error we initially throw
          // but there is no clean path to get it here. Could use a refactor.
          throw Errors.INVALID_REMOTE_CAPABILITY()
        }
        safetyCatch(err)
        this._pendingRPC = null
