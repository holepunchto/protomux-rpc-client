...
const BucketRateLimiter = require('./bucket-rate-limiter.js')
...
class ProtomuxRpcConnection extends SuspendResource {
  constructor (
    ...
    this._requestBucketRateLimiter = rateLimit ? new BucketRateLimiter(rateLimit) : null
  ...
  async _close () {
    ...
    if (this._requestBucketRateLimiter) this._requestBucketRateLimiter.destroy()
  }
  ...
  async makeRequest (methodName, args, { requestEncoding, responseEncoding, timeout = 10000 } = {}) {
    console.log('makeRequest', methodName, args)
    if (!this.opened) await this.ready()
    ...
    try {
      const executeFn = () => this._requestConcurrentLimiter.execute(
        () => this._connectAndSendRequest(
          methodName,
          args,
      ...
      if (this._requestBucketRateLimiter) {
        return await this._requestBucketRateLimiter.execute(executeFn, { abortSignalPromise })
      } else {
