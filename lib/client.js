...
class ProtomuxRpcConnection extends SuspendResource {
    ...
    try {
      return await this._requestConcurrentLimiter.execute(async () => {
        return await this._connectAndSendRequest(
          methodName,
          args,
          { requestEncoding, responseEncoding, rpcTimeout: timeout, abortSignalPromise: timeoutSignal.wait() }
        )
  ...
  async _connectAndSendRequest (methodName, args, { requestEncoding, responseEncoding, rpcTimeout, abortSignalPromise }) {
    ...
    while ((!this.rpc || this.rpc.closed) && !this.closing) {
      const connectedPromise = this.connect()
      connectedPromise.catch(safetyCatch)
      ...
      const suspendedPromise = this._waitUntilNotSuspended()
      suspendedPromise.catch(safetyCatch)
